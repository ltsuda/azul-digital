...
public final class PhotoActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private int reason;
    private String mapAdress, plate, fiscal;
    private File file12;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            if (getIntent().getStringExtra("map_address") != null) {
                mapAdress = getIntent().getStringExtra("map_address");
            }
            ...
            nextEmail = (Button) findViewById(R.id.next_email);
            findViewById(R.id.next_email).setOnClickListener(this);
            nextEmail.setEnabled(false);
            nextEmail.setAlpha(0.5f);
    ...
    final CameraCaptureSession.CaptureCallback captureCallbackListener = new CameraCaptureSession.CaptureCallback() {
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request, TotalCaptureResult result) {
            ...
            Toast.makeText(PhotoActivity.this, "Salvo" + file12, Toast.LENGTH_SHORT).show();
            createCameraPreview();
        }
    };
        ...
        try {
            ...
            ImageReader.OnImageAvailableListener readerListener = new ImageReader.OnImageAvailableListener() {
                ...
                private void save(byte[] bytes) throws IOException {
                    file12 = getOutputMediaFile();
                    OutputStream output = null;
                    try {
                        output = new FileOutputStream(file12);
                        output.write(bytes);
                    } finally {
                        if (null != output) {
                            output.close();
...
//                    Toast.makeText(PhotoActivity.this, "Salvo:" + file12, Toast.LENGTH_SHORT).show();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults[0] == PackageManager.PERMISSION_DENIED) {
                // close the app
                Toast.makeText(PhotoActivity.this, R.string.permissao_camera, Toast.LENGTH_LONG).show();
                finish();
            }
        }
    }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.getpicture) {
            // launch Ocr capture activity.
            System.out.println("open camera");
            getPicture();
            nextEmail.setEnabled(true);
            nextEmail.setAlpha(1.0f);

        } else if (view.getId() == R.id.next_email) {
            Intent intent = new Intent(this, EmailActivity.class);
            intent.putExtra("map_address", mapAdress);
    ...
    private static File getOutputMediaFile() {
        ...
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.d("MyCameraApp", "falha ao criar pasta");
                return null;
            }
        }
